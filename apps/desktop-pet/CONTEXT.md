...
**位置钳制 (Window Clamp)**:
防宠物拖出屏幕丢失的位置约束，纯函数在 `window-clamp.ts`。边界是显示器**完整边界**（含任务栏，宠物允许坐任务栏），多显示器取并集。分两档：**软钳制**（拖动中）只保 32×32 最小可见条、保留贴边玩法；**硬钳制**（启动/设置恢复）把精灵完整拉回坐标所在显示器（不命中任何屏则主显示器），治愈分辨率变更/拔屏后的旧档。
_Avoid_: 工作区钳制（会禁止坐任务栏）, 窗口级钳制（透明边距导致"可见"误判）
